...
class AddQuotationModel extends BaseViewModel {
  ...
  Future<void> initialise(BuildContext context, String quotationid) async {
    ...
    if (QuotationId != "") {
      ...
      isEdit = true;
      quotationdata = await AddQuotationServices().getquotation(quotationid) ??
          AddQuotation();
      ...
      searchcustomer = await AddQuotationServices()
          .getcustomer(quotationdata.quotationTo ?? "");
  ...
  void setSelectedItems(List<Items> SelectedItems) async {
    ...
    Logger().i(quotationdata.toJson());
    quotationdetails(
        await AddQuotationServices().quotationdetails(quotationdata));
    notifyListeners();
  }

  void quotationdetails(List<QuotationDetailsModel> quotationdetail) {
    isSame = false;
    quotationdata.totalTaxesAndCharges = (quotationdetail.isNotEmpty
        ? quotationdetail[0].totalTaxesAndCharges
        : 0.0);
